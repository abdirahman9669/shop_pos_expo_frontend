...
export default function NewSale() {
  ...
  const [sosRate, setSosRate] = useState('27000');
  ...
  const fetchCurrencies = useCallback(async (): Promise<Currency[]> => {
    try {
      const r = await fetch(URLS.currencies, { headers: authHeaders });
      const j = await r.json();
      const arr: any[] = j?.currencies ?? j ?? [{ code: 'USD' }, { code: 'SOS' }];
      return arr.map(c => ({ ...c, id: c.code }));
    } catch {
      return [{ id: 'USD', code: 'USD' }, { id: 'SOS', code: 'SOS' }];
    }
  }, []);

  const fetchAccounts = useCallback(async (): Promise<Account[]> => {
    const r = await fetch(URLS.accounts, { headers: authHeaders });
    const j = await r.json();
    return (j?.data ?? j ?? []);
  }, []);

  const fetchCustomers = useCallback(async (): Promise<Customer[]> => {
    const r = await fetch(`${URLS.customers}?limit=50`, { headers: authHeaders });
    const j = await r.json();
    return (j?.data ?? j ?? []);
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [devs, sess, sts, curs, acs] = await Promise.all([
          fetchDevices(), fetchSessions(), fetchStores(), fetchCurrencies(), fetchAccounts()
        ]);
        ...
        if (curs?.length && !currency) setCurrency(curs[0]);

        const pick =
          acs.find(a => a.AccountType?.name === 'CASH_ON_HAND' && (currency?.code === 'USD' ? /USD/i : /SOS/i).test(a.name)) ||
          acs.find(a => a.AccountType?.name === 'CASH_ON_HAND') ||
          acs[0];
  ...
  /** ---------- Scan (camera stays OPEN) ---------- */
  const [lastScanCode, setLastScanCode] = useState<string>('');
  const lastScanTimeRef = useRef(0);
  const lastHandledByCodeRef = useRef(new Map<string, number>()); // per-code cooldown
  const handlingRef = useRef(false); // serialize handling (avoid race on add)

  useEffect(() => { if (scanOpen && !permission?.granted) requestPermission(); }, [scanOpen, permission?.granted, requestPermission]);

  // Add or bump +1 (returns new qty)
  ...
  const onScanned = useCallback(async (e: { data: string }) => {
    ...
    // global serialize: one scan handled at a time
    if (handlingRef.current) return;
    handlingRef.current = true;

    // per-code debounce: ignore same code within 800ms
    const now = Date.now();
    ...
    // also store last scanned (optional visual/debug)
  ...
  const setQty = (id: string, v: string) =>
    setLines(prev =>
      prev.map(l =>
        l.product_id === id
          ? { ...l, qty: Math.max(1, Math.floor(Number(v || '1'))) }
          : l
      )
    );
  ...
  const totalUsd = useMemo(() => lines.reduce((s, l) => s + (l.qty * l.unit_price_usd), 0), [lines]);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const method =
        account?.name?.match(/USD/i) ? 'CASH_USD'
        : account?.name?.match(/SOS/i) ? 'CASH_SOS'
        : (currency.code === 'USD' ? 'CASH_USD' : 'CASH_SOS');
      ...
      const body = {
        ...
        pay: {
          ...
          amount_usd: n(amountPaidUsd, 0),
          ...(currency.code === 'SOS' ? { customer_rate_used: n(sosRate, 0) } : {}),
        },
        status: 'COMPLETED',
      };

      const r = await fetch(URLS.sales, { method: 'POST', headers: authHeaders, body: JSON.stringify(body) });
      const j = await r.json();
  ...
  }, [device, session, store, customer, lines, account, currency, amountPaidUsd, sosRate]);
        ...
        ListHeaderComponent={
          <View style={s.wrap}>
            ...
            <PickerField<Device>
              label="Device"
              value={device}
              onPick={(v) => { setDevice(v); setSession(null); }}
              fetchList={fetchDevices}
              display={labelOfDevice}
            />
            <PickerField<CashSession>
              label="Cash Session"
              value={session}
              onPick={setSession}
              fetchList={async () => {
                const list = await fetchSessions();
                return device ? list.filter(s => s.device_id === device.id) : list;
              }}
              display={(s) => new Date(s.opened_at).toLocaleString()}
            />
            <PickerField<Store>
              label="Store"
              value={store}
            ...
            <View style={s.grid2}>
              <View>
                <Text style={s.subLabel}>Paid (USD)</Text>
                <TextInput
                  value={amountPaidUsd}
              ...
              {currency?.code === 'SOS' && (
                <View>
                  <Text style={s.subLabel}>Customer Rate (SOS/USD)</Text>
                  <TextInput
                    value={sosRate}
                    onChangeText={setSosRate}
                    keyboardType="number-pad"
                    style={s.input}
                    placeholder="27000"
                  />
                  <Text style={{ color: '#666', marginTop: 4 }}>
                    Native (SOS): {(n(amountPaidUsd, 0) * n(sosRate, 0)).toFixed(2)}
                  </Text>
                </View>
              )}
            </View>
